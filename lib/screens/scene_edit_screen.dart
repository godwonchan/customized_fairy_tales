...
class _SceneEditScreenState extends State<SceneEditScreen> {
  ...
  Future<void> _generatePreviewFlow() async {
    if (widget.tale.storyId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('서버 동화만 수정할 수 있어요.')),
      );
      return;
    }
    ...
    if (!_hasSketch) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('먼저 그림판에 낙서를 그려주세요.')),
      );
      return;
    }
  ...
  Future<void> _regenerateFromChat() async {
    ...
    if (_highlights.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('먼저 선택된 텍스트가 있어야 해요.')),
      );
      return;
    }
